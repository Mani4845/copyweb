...
<div className="text-white font-['Neue Montreal']">
  <Link to="/">
    <svg
      ...
      height="30"
      viewBox="0 0 72 30"
      fill="currentColor"  // Ensure color matches the background
    ...
    >
      ...
      <div className="hidden md:flex gap-10">
        {menuItems.map((item, index) => (
          <Link
            key={index}
            to={item.path}
            className={`text-md font-light text-white shadow-lg ${
              index === 4 && "ml-28"
            }`}
          >
            {item.name}
      ...
      <div
        className={`md:hidden ${
          isOpen ? "block" : "hidden"
        } absolute top-16 left-0 w-full bg-zinc-900 shadow-lg`}
      >
        {menuItems.map((item, index) => (
          <Link
            key={index}
            ...
            onClick={toggleMenu} // Close the menu after navigation
          >
            {item.name}
          </Link>
        ))}
