...
function Navbar() {
  ...
  const menuItems = [
    ...
    { name: "Services Us", path: "/Services-us" },
  ...
  return (
    <div
      ...
      data-scroll-section
      data-scroll-speed=".1"
      className=" z-[999] w-full px-5 py-4 flex justify-between items-center bg-zinc-900 shadow-md"
    >
      {/* Logo */}
      <div className="text-white font-['Neue Montreal']">
      <Link to="/">
        <svg
          width="72"
          height="30"
      ...
      <div className="hidden md:flex gap-10">
        {menuItems.map((item, index) => (
          <Link
            key={index}
            to={item.path}  // Use Link's to attribute instead of href
            className={`text-md font-light text-white shadow-lg ${index === 4 && "ml-28"}`}
          >
            {item.name}
          </Link>
        ))}
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden ${isOpen ? "block" : "hidden"} absolute top-16 left-0 w-full bg-zinc-900 shadow-lg`}
      >
        {menuItems.map((item, index) => (
          <div
            key={index}
            className="block px-4 py-2 text-md font-light border-b text-white cursor-pointer"
            onClick={() => {
              toggleMenu();
              window.location.href = item.path;  // Optional: You can use Link here too
            }}
          >
            {item.name}
          </div>
